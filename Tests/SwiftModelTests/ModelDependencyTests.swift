...
struct ModelDependencyTests {
    @Test func testChildDependency() async {
        ...
        await withModelTesting {
            let model = Child(id: 2).withAnchor {
                $0.testResult = testResult
                $0[Dependency.self] = Dependency(value: 4)
            }
            await expect(model.dependency.value == 4)
        }
    ...
    @Test func testParentChildDependency() async throws {
        ...
        await withModelTesting {
            ...
            await expect(model.child.dependency.value == 4)
        }
    ...
    @Test func testParentChildMultiDependency() async throws {
        ...
        await withModelTesting {
            ...
            await expect(model.children.isEmpty)
    ...
    @Test func testSharedDependency() async throws {
        ...
        await withModelTesting {
            ...
            await expect(model.children[0].dependency.value == 8)
...
@Model private struct Child {
    ...
    func onActivate() {
        ...
        node.forEach(Observed(initial: false, coalesceUpdates: false) { dependency.value }) {
