...
struct DriveQoSDiagnosticsTests {
    @Test func whereDoDriveJobsAndYieldsRun() async {
        ...
        print("DIAG drainQueueQoS=main-unspecified cores=\(ProcessInfo.processInfo.activeProcessorCount) testThreadQoS=\(qos_class_self().rawValue) testQueue=\(String(cString: __dispatch_queue_get_label(nil)))")
