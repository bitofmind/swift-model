...
public extension Exhaustivity {
    ...
    /// Require all metadata state changes (via `node.metadata`) to be consumed by `assert` blocks.
    internal static let metadata = Self(rawValue: 1 << 4)

    /// Exhaustivity is completely disabled — no side effects need to be asserted.
    static let off: Self = []
    /// All categories are checked. This is the default.
    static let full: Self = [.state, .events, .tasks, .probes, .metadata]
}
