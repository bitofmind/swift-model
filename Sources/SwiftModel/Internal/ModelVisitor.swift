...
public protocol ModelVisitor<State> {
    ...
    // MARK: - _ModelState-level dispatch (SPI)

    /// Whether this visitor needs witness-table dispatch for `_ModelState`-level paths (paths into
    /// the `_State` struct generated by `@Model`) via `_visitModelStatePath`, rather than the
    /// `visit(statePath:)` composition.
    ///
    /// This replaces the former `modelVisitor as? any _ModelStateVisitor` runtime existential check
    /// in `ContainerVisitor._dispatchStatePath`. That `as?` routed through
    /// `swift_conformsToProtocol`, which has null-dereferenced inside `libswiftCore` on Linux when
    /// the runtime conformance cache races first-time instantiation (the `Linux (serial)` SIGSEGV in
    /// `swift_conformsToProtocolMaybeInstantiateSuperclasses`). A plain witness-table requirement is
    /// dispatched directly off the conformance record `ContainerVisitor` already holds for `V`, so it
    /// never triggers a conformance-cache lookup — and is cheaper than a per-property `as?` scan.
    ///
    /// Default `false`; only `InstallUndoVisitor` returns `true`.
    var _isModelStateVisitor: Bool { get }

    /// Visits a `_ModelState`-level path on a `_isModelStateVisitor` visitor.
    ///
    /// `forState` carries the concrete `M` (always `V.State`) so the conformer can recover the
    /// `M`-specific path type. Default is a no-op; only `_isModelStateVisitor` visitors override it.
    mutating func _visitModelStatePath<M: Model, T>(_ statePath: WritableKeyPath<M._ModelState, T>, forState: M.Type)
    mutating func _visitModelStatePath<M: Model, T>(_ statePath: WritableKeyPath<M._ModelState, T>, forState: M.Type, visibility: PropertyVisibility)
}
...
public extension ModelVisitor {
    ...
    var _isModelStateVisitor: Bool { false }
    mutating func _visitModelStatePath<M: Model, T>(_ statePath: WritableKeyPath<M._ModelState, T>, forState: M.Type) { }
    mutating func _visitModelStatePath<M: Model, T>(_ statePath: WritableKeyPath<M._ModelState, T>, forState: M.Type, visibility: PropertyVisibility) { }
}
