...
/// The fields the model read and write paths consult on every access are
/// `@exclusivity(unchecked)`. A stored property on a class is otherwise guarded by Swift's
...
/// `withForceDirectAccess`, `withUntrackedModelReads`) does a plain get, a plain set, runs
/// its body, then a plain set; nothing holds a `_modify`/`inout` access across a call, and
/// no accessor yields into user code. Dropping the check changes nothing observable.
/// Fields off both paths keep the default checking.
@usableFromInline
