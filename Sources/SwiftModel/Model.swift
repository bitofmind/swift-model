...
public protocol Model: ModelContainer, Identifiable, Sendable {
    ...
    /// Framework internal. The concrete state type for macro-generated models.
    /// Defaults to `_EmptyModelState` for non-macro types. Do not use directly.
    associatedtype _ModelState: _ModelStateType = _EmptyModelState
    ...
    /// Framework internal. Returns `\Self._modelState` formed in the **concrete** type's context.
    ///
    /// Forming a keypath to `_modelState` in a generic context (`M: Model`) uses
    /// protocol-witness-table dispatch, producing a keypath component that is **not equal** to one
    /// formed with direct dispatch in the concrete type. Storing the keypath as the return value of
    /// a concrete-type method and reading it generically preserves the concrete-context
    /// representation, so `M._modelStateKeyPath.appending(path: statePath)` equals
    /// `\._modelState.propertyName` from `visit(with:)`. Used by `Context` to compose observation
    /// keypaths correctly. Generated by `@Model` macro; private `_modelState` is inaccessible
    /// to generic framework code directly.
    static var _modelStateKeyPath: WritableKeyPath<Self, _ModelState> { get }
...
public extension Model {
    ...
    var node: ModelNode<Self> { ModelNode(_$modelContext: _context._$modelContext, model: self) }
}

// Default for types with no tracked mutable state (_ModelState = _EmptyModelState).
// @Model types with tracked vars generate _modelStateKeyPath in the concrete type via the macro.
// This default is never actually called — it exists solely so the requirement is satisfied.
extension Model where _ModelState == _EmptyModelState {
    public static var _modelStateKeyPath: WritableKeyPath<Self, _ModelState> { fatalError() }
}
