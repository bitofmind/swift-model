...
- **A stable per-property index on every `@Model`'s `_State` — framework-facing groundwork, not API.** The macro now emits `_trackedPropertyCount` and `_trackedPropertyKeyPaths` on the generated `_State` struct: a zero-based index per tracked property in declaration order, covering exactly the fields of `_State` and nothing else (`let` properties, `@ModelDependency`, `@_ModelIgnored` and computed properties have no index, as they have no field). `_ModelStateType` gains all three as requirements, plus `_trackedPropertyIndex(of:)` to map a `_State` key path back to its index; `_EmptyModelState` and non-macro conformers report `0` / `[]` / `nil`. Nothing in the library reads the table yet and no accessor, `_ModelSourceBox` or `Context` code path changed. It is the table the follow-up read/write-path work keys on: today every tracked access passes a `\_State.prop` literal, whose `_swift_getKeyPath` retains one process-wide object from every core (the 8-thread cliff the contention probe measures), and both the observer-KP cache and `modifyCallbacks` hash that object — a per-context table indexed by a small integer the accessor passes as a literal removes all three. The members are computed rather than `static let`s because `PartialKeyPath` is not `Sendable` and Swift forbids static stored properties in generic types (a generic `@Model`, or one nested in a generic type, makes `_State` generic), so `_trackedPropertyIndex(of:)` is an identity-then-structural scan of the key-path array — a registration-time cost only, never per access. Under `defaultIsolation: MainActor` the members inherit `_State`'s `nonisolated`.

### Changed

- **A tracked `@Model` property read is now ~380 ns instead of ~730 ns on the main thread, and an untracked one ~135 ns instead of ~220 ns (M1 Max, Release), from four fixed costs the read path paid on every access.** First, the read was a `_read` coroutine nested inside another (`_ModelSourceBox.subscript(read:access:)` yielding straight out of `Context.subscript(statePath:observeCallback:)`), and the combined yield-once frame did not fit the caller's coroutine buffer, so every anchored read heap-allocated it (`swift_coroFrameAlloc`, a malloc/free pair). Both are plain getters now: the Context-level read (`Context.readLocked`) always materialised the projection into a local before yielding — that is the exclusivity discipline, and it is kept — so returning it by value is the same copy without the frame. Second, `withUntrackedModelReads` reads resolved the live `Context` through the Reference's weak `_context` (`swift_weakLoadStrong` plus the strong retain/release it hands back, ~15% of an untracked read in the 8-thread profile) only to take its lock; `Reference.readUntracked` now takes the hierarchy lock the Reference already holds strongly and never loads the context. The tracked path still needs the Context object (registrar, observer-KP identity, `activeAccess.willAccess`) and still loads it once — `_context` stays `weak`, because an `unowned(unsafe)` pointer can be loaded after the refcount hit zero but before `Context.deinit` clears the field, and an unlocked weak load racing `setContext`/`clearStateForGeneration` is not something the runtime promises to be safe. Third, a tracked read on the main thread re-took the hierarchy lock a second time to fetch the lazily-created main `ObservationRegistrar`, and paid an `as!` on the registrar box each time; the main/background choice now lives in one accessor (`currentThreadObservationRegistrar`), which makes the main thread the only writer of `_main` by construction — so its own read of it needs no lock (there is no second thread for a data race to involve; off-main readers still lock) — and the cast is an `unsafeDowncast` of a `let` that is only ever assigned a `RegistrarBox`. Fourth, `threadLocals` (a `pthread_getspecific` plus an `Unmanaged` retain/release) was fetched up to six times per tracked read and each flag behind it paid a dynamic exclusivity check; it is resolved once and passed down, and the read-path fields of `ThreadLocals`, `Reference` and `AnyContext` are `@exclusivity(unchecked)` with the invariant that makes that sound written at each declaration (thread-confined or lock-protected, and never held open by a `_modify`).

  Best of 3, ns per op: section 2 `property read` 726 → 380; section 2b `tracked read (no listener)` 727 → 382, `tracked read (inside withObservationTracking)` 871 → 514, `untracked read (inside scope)` 220 → 134, `withUntrackedModelReads { single read }` 229 → 139; section 2c `tracked scan` (120 children × 2 properties) 183.0 → 99.3 μs, `untracked scan` 58.3 → 38.5 μs, `tracked scan (inside withObservationTracking)` 331 → 237 μs, `value-snapshot scan` unchanged (1.56 → 1.51 μs); the in-module probe 679 → 344 tracked and 214 → 94 untracked. `--contention`, ns per op per thread at 1T/2T/4T/8T: `tracked read distinct trees` 473/545/1311/3375 → 345/430/1037/4156, `untracked read distinct trees` 217/226/598/1889 → 136/155/597/2125, `tracked read children of ONE tree` 494/591/2106/6990 → 346/432/1656/4621. The 8-thread distinct-tree rows did not improve and the tracked one is worse: with less local work per read, eight threads hit the two process-global key-path objects (the `\_State.count` literal `_swift_getKeyPath` hands out, and the `_stateObserverKP` cache entry) at a higher rate, and contended refcounting on a shared cache line grows superlinearly with that rate — the 8T profile shows malloc/free, the weak load's retain, `threadLocals.getter` and `swift_beginAccess` gone from the top of stack and `swift_retain`/`swift_release` under `_swift_getKeyPath` / `_stateObserverKP` up in their place. Those two objects are the macro-index follow-up's to remove; nothing in this change touches them. What a reader notices: every O(N) traversal that reads model properties runs close to twice as fast, in the app (SwiftUI `body`, reconciliation walks) and in tests; behaviour is unchanged — observation registration, `withUntrackedModelReads` (still lock-protected, still unobserved), destructed / last-seen reads, the transitions-mode override, memoize/`Observed` dependency collection and `TestAccess` all see exactly the same calls in the same order.

- **A property write costs about half of what it did, and eight threads writing to eight independent models now scale instead of serialising on process-wide state.** A tracked `@Model` property write with no observers measured 1.6 µs on the main thread (1.2 µs off it) on an M1 Max in Release; it is now 0.9 µs (0.68 µs off main). On the contention probe, `c4 write distinct trees` went from 1232 / 1523 / 3065 / 7923 ns per write per thread at 1 / 2 / 4 / 8 threads to 679 / 737 / 784 / 3101 — flat to four threads where it used to be 2.5× worse, and 2.6× better at eight — and `c5 1 writer + N-1 tracked readers distinct` from 1240 / 1378 / 1893 / 3933 to 684 / 719 / 801 / 3487. A write with an `onChange` observer dropped from 4.7 µs to 3.0 µs, and every hierarchy-mutation row (section 5) improved 3–9 % as a side effect. Nothing about what a write *does* changed — the same registrar `willSet`/`didSet` sequence, the same `isSame` short-circuit, the same post-lock callback order and `lockHeldBackgroundCalls` deferral — only what it costs. Where the time went, per write:

  - *Hashing a key path to discover there was nothing to find.* The no-observer fast path in `buildPostLockCallbacks` began with `modifyCallbacks[path] != nil`, which on an empty store ran the `_read` accessor's `?? [:]` — a `Dictionary.init(dictionaryLiteral:)` in unspecialised generic code that instantiates the `(key:value:)` tuple metadata through the runtime's locked cache — then copied the dictionary, fetched the `Hashable` witness for `PartialKeyPath<M._ModelState>` and hashed the key path structurally. The context now keeps a `Bool` beside the store (`hasModifyCallbacks`, maintained by the store's only mutation route); the hash happens only once some `onModify` is actually registered.
  - *Two coroutine frames and a closure box.* A tracked write was a `_modify` in `_ModelSourceBox` yielding into a second `_modify` on `Context`. Each generic `_modify` allocates its frame on the heap (`swift_coroFrameAlloc` — a malloc and a free per frame; on the current allocator the free alone includes a `mach_absolute_time` call), and the sameness check crossed that coroutine boundary as an `((T, T) -> Bool)?` — implicitly escaping, so the `Equatable` overload's `==` thunk was a `swift_allocObject` per write. `Context` now exposes `beginDirectWrite` / `endDirectWrite` / `closeDirectWrite` and the caller yields its own local: one coroutine, one frame, `==` applied directly. Whole-value assignment (`isLoading = true`) gets a plain function, `setValue`, with no coroutine at all. Tuple (parameter-pack) properties keep the two-coroutine shape, because Swift 6.3's SILGen crashes lowering the caller-owned form for a `(repeat each T)` value.
  - *Dynamic exclusivity bookkeeping on thread-confined and lock-protected state.* Every access to a class stored property goes through `swift_beginAccess`/`swift_endAccess` — a per-thread access-set lookup and insert, ~25 ns — and a write touched about a dozen of them across `ThreadLocals` and the context's counters. Those properties are now `@exclusivity(unchecked)`: the check only ever catches an overlapping access on the *same* thread, and none of these has an accessor that yields into user code while an access is open, so there is nothing for it to catch.
  - *Process-wide refcount traffic.* Opening the `lockHeldBackgroundCalls` scope stored an empty array into a thread-local and cleared it again — a retain/release pair on the process-wide empty-array singleton, one cache line every writing thread contends on. The scope is now a `Bool`; the array is created on first deferral. The registrar box was reached through `as?` (a `swift_dynamicCastClass` plus a retain/release of the box) up to four times per write, and the main registrar through a redundant re-entry of the hierarchy lock the write already held; both are now a borrowed read.
  - *Metadata resolved twice, task-locals read twice.* `didSet` lived in a `defer` — a separate function that re-resolved the `_StateObserver<M._ModelState>: Observable` witness table; the same sequence is now written out inline. The `& Sendable` view of the key path (an existential metadata lookup, plus an `Optional` copy of it) is formed only when a `TestAccess` or collector is there to receive it. `ModelAccess.active` / `ModelAccess.current` and the `threadLocals` slot are read once per write and carried down (`finishWrite`, `invokeDidModifyDirect`, `buildPostLockCallbacks`).

  What is left in a write, in order: Apple's registrar `willSet`/`didSet` (its own lock and `Set<AnyKeyPath>` bookkeeping), the one remaining coroutine frame, the `_stateObserverKP` stripe lock — the dominant cost at eight threads, and the subject of the next PR — the `Reference.context` weak load, and the generic metadata for the registrar's subject type.
...
- **Fixed a data race on `TaskCancellable.hasStartedRunning`.** The `LockIsolated<Bool>` box behind it was a `var` installed after `self.init` under the instance lock, but read *without* that lock, on the theory that a reader racing the install sees `nil` and reports the safe default (`false` — settle keeps waiting). The value was indeed always safe; the reference read was not, and it is a data race whatever it returns. TSan caught it on this PR's run: `TestExecutorDrive._driveToStableFixpoint` → `AnyContext.hasPendingStartTask` → `Cancellations.hasPendingStartTask` reading the field while a `forEach` task registration wrote it. Latent since 2026-05-22 and only reachable in volume once the drive began polling `hasPendingStartTask` on every fixpoint pass; the loop-stress test fixes in this PR (30 and 40 *live* hierarchies where only one used to be anchored) raised the hit rate enough to surface it. The box is now created by the convenience init, passed into the designated init, and held as a `let` assigned before `cancellations.register(self)` publishes the instance — the window is removed rather than defended, and the observable value is unchanged.
...
- **External cancellation no longer masquerades as a `[TRAIT timeout]`.** `_withTestTimeout`'s parked watchdog children (the inactivity window and the absolute ceiling) also unpark when the task is *cancelled* — an external kill such as `xcodebuild`'s per-test allowance, or a parent teardown — and used to race the body's slower cancellation unwind through `group.next()`. When a watchdog won, it stamped a spurious `[TRAIT timeout]` (or an absolute-ceiling report claiming the full ceiling span) over the real cause; observed in the wild as a "ran 1500 s" ceiling message inside a test run that was only 280 s old, and as dozens of echoed `[TRAIT timeout]`s in a run whose true failures were `Time limit was exceeded: 120 seconds` allowance kills. Watchdog children now return a verdict-free `.cancelled` when their unpark was cancellation, the consumer skips those and keeps waiting for the body, and the body's own `CancellationError` (or value) surfaces the real outcome. `externallyCancelledTraitCapDoesNotReportTimeout` pins it; the genuine window/ceiling paths are unchanged and stay covered by the existing timeout meta-tests.

- **The drive's "never reached a fixpoint" verdict is now evidence-based, not wall-clock.** The old design failed a settle at a fixed 120 s watchdog — a wall-clock discriminator that conflated two opposite conditions: a genuine runaway (which deserves to fail *fast*) and a healthy wait merely slowed by load (which must not fail *at all*). The same downstream field report exposed both edges at once: six test bundles under parallel `xcodebuild` starved the one shared main thread that the fixpoint check's observation drains empty onto, pushing perfectly healthy settles past 120 s into false "never reached a fixpoint" failures. Scaling the watchdog would have been another round of timeout-hunting; instead the discriminator is now **work, not time**, in the same spirit as the drive itself:

  - **Runaway = unbounded same-site reactive work.** A wait fails when a single reactive call-site (`node.onChange` / `node.forEach`) accumulates `_settleRunawayFireBound` deliveries (50 000; meta-tests override it) *within that one wait* while still firing — evidence no healthy wait can produce, because a healthy wait's pending work is bounded no matter how slowly load lets it drain. Load can only make the verdict *later*, never *wrong*, and interactively it lands in milliseconds instead of 120 s (the meta-test's feedback loop fails in ~0.1 s, with the runaway callout naming the offending call-site). This is Update 26's runaway *diagnostic* promoted to the *trigger* — and wiring it up exposed that `_onChangeImpl`'s delivery loops never called `reactiveBodyFired` (only `_forEachImpl` did), so an `onChange` feedback loop was invisible to the callout too; both loops are now instrumented.
  - **A healthy-but-slow wait now just waits.** No wall-clock verdict remains on the fail path. The only remaining deadline is a last-resort *termination* ceiling (2× the trait cap's absolute ceiling, derived from the trait's own constants rather than a new free number) covering contexts with no trait cap (`withModelTesting`) and non-reactive stalls; wherever the trait is present, its more specific inactivity-watchdog/ceiling diagnostics always fire first — and settle stays silent on that cancellation (see the entry above).
  - **The trait cap's inactivity watchdog now sees every progress conduit, so it stops mis-killing main-starved healthy tests.** Its probe was executor activity alone; a test legitimately parked on a slow main-thread drain showed "no activity" for the whole stall and was cut down at the cap — the trait-timeout cascade in the field report. The probe is now the union of executor activity and the test's `progressNs` (model writes/events, main- and background-queue drain stamps — the same conduits the fixpoint check waits on), so only a test where *nothing of its own* moves for a full window trips the cap.

  `DriveRunawaySettleTests` pins the runaway verdict end-to-end (feedback loop → evidence-based fail → quench → settle recovers); `TraitProgressSignalTests` pins each progress conduit; `driveBoundsKeepTheirOrderingAtEveryScale` pins the scale-invariant orderings (trait window < `waitUntil` backstop < trait ceiling < drive termination ceiling).

- **`settle`'s fixpoint grace now scales with `SWIFT_MODEL_TIMEOUT_SCALE`, matching `_expectGraceNs`.** The 30 ms quiet window that decides "the model is done" was the one quiescence measurement that didn't scale: on an environment whose execution is uniformly slower (the TSan job's 5–15× slowdown on a 2–3-core runner), a normal mid-chain scheduling hop of a few ms spans tens of wall-clock ms and read as genuine quiet, letting `settle` declare a fixpoint with a child's chain still in flight (the `settleIsLoadIndependentAcrossChildTasks` TSan-CI flake). Measurement calibration, not a deadline — settle still never fails on wall-clock; at scale 1 nothing changes.
...
- **`TaskCancellable.init` no longer resolves `context.cancellations` while holding its own lock, closing an AB-BA deadlock between the hierarchy lock and a task's `NSLock`.** The capture-list expression `[weak cancellations = context.cancellations]` sat *inside* `lock { }`, and capture lists are evaluated at closure-formation time — i.e. inside the enclosing critical section. Since `AnyContext.cancellations` takes the per-context hierarchy lock (**H**) and the instance lock is **T**, registering a task ran **T→H**. Teardown runs the opposite order: a subtree replacement reaches `AnyContext.onRemoval` → `Cancellations.cancelAll()` → `TaskCancellable.onCancel`, which blocks on T — **H→T**. Two threads, opposite orders, permanent hang at 0% CPU. (That arm needs care to state correctly: `AnyContext.onRemoval()` drains its callbacks *outside* its own `lock`, so `cancelAll` looks unlocked and the arm looks refuted. What supplies H is the enclosing `node.transaction`, which holds it across its entire body.) The registry is now resolved once into a local before the lock and captured from there, making the order uniformly H-before-T; this costs nothing, since `init` already needed that value on its first line.
  ...
  Fourth in the family after the `reduceHierarchy` (1.0.6) and `memoize` first-access (1.0.7) inversions, but **materially different in reach**: those two inverted against the `TestAccess` write lock, which does not exist in production (`ModelAccess.acquireWriteLock()` is a no-op there), so they could only ever wedge a test plan. This pair is the hierarchy lock and a plain `NSLock`, **both of which exist in a shipping app** — so this was a latent production hang. It remains *unobserved* in production; the test drain executor raises the odds by tearing down several models' tasks at once.

  Landed on the **cycle**, not on a rate: the inversion is verifiable by reading the two call sites, and a wedged process was captured (0% CPU, two `sample`s 44 s apart on identical frames). A downstream A/B initially measured 4 wedges in 51 unfixed runs against 0 in 90 patched, but the unfixed arm later failed to reproduce on re-run, so **those counts are deliberately not cited as evidence** — a lock-order cycle and a captured deadlocked stack are not rate estimates. `CancellableLockOrderDeadlockTests` is a concurrency smoke test in the manner of its two predecessors; the durable guard is the ordering itself.

  Worth recording for whoever meets this class next: swift-model's own inactivity watchdog **cannot** catch it. `_DrainTestExecutor.activityNs` reports `now` whenever `outstanding > 0`, and deadlocked jobs stay outstanding forever — so the mechanism built to catch hangs is blind to this one. Only an out-of-process timeout (e.g. `xcodebuild`'s `-default-test-execution-time-allowance`) fires.

- **`checkExhaustion` now deep-copies the two state trees under `TestAccess.lock` before diffing them, fixing a data race in the test harness.** It already read `expectedState` / `lastState` out under the lock — but copying the *root* is not enough. `didModify` updates `lastState` by mutating the existing frozen tree **in place** (`_writeToFrozenState`, installing freshly-built frozen child references), so a root copy still shares those mutable `Reference` objects. The diff must run *outside* the lock (`diffMessage` walks state via `customMirror`, which re-enters the lock and would deadlock cross-thread), so it was walking a tree another thread was still editing — reading `Reference`s published *after* our lock acquire, hence with no happens-before edge to us. ThreadSanitizer reports it as `Reference.context.getter` / `Reference.isSnapshot.getter` racing `Reference.init` under `_ModelSourceBox.init(frozen:id:)`.

  `frozenCopy` rebuilds every node while preserving each `modelID`, so layer 2's identity-only diff is unaffected. Cost is one deep copy per exhaustion check (per `expect`), versus `didModify`'s existing per-write copy of just the changed value — the cheaper side of the trade, since writes outnumber `expect`s.

  **Unlike #48 and #49, this one is proven against a reproduction.** A probe driving 180 reactive `expect` resolutions against three concurrent OS-thread writers, under `--sanitize=thread`: **9/9 unfixed runs reported it (74 reports total, 3–14 per run); 6/6 fixed runs are clean.** The unfixed arm was re-confirmed at the end of the session, after the fixed runs, so the difference is the change and not the machine conditions. Test-harness only; no library behaviour change.

- **`_ModelSourceBox.subscript(dynamicMember:)` now projects state under the hierarchy lock, closing a use-after-free on the *read* path.** The getter checked `reference._stateCleared` and then evaluated `reference.state[keyPath: path]` with no lock at all, while `Reference.clear(ifGeneration:)` swaps `state` for `_genesisState` under that lock. This is the same **memory-safety** bug class as the write-path fix below, mirrored onto reads: a key-path projection loads the slot and only *then* retains what it loaded, so a clear that drops the old state's last reference in between hands that retain an already-freed object. The `_stateCleared` check and the projection now happen in one locked scope — splitting them only moves the TOCTOU — and `reportIssue` stays outside the lock, since it can re-enter model reads. Snapshot references short-circuit before the lock: `clear()` only ever runs against a Context's own reference, a snapshot has none, and `_snapshotLifetime` is an immutable `let`.

  ThreadSanitizer had only ever reported the `_stateCleared` / `_hasGenesis` halves of this, because those are plain `Bool`s it can see; the dangerous projection on the next line is inside uninstrumented `libswiftCore` and structurally invisible to it — the same blind spot documented for the write path.
...
- **`expect`'s post-await reads of its `EvalSnapshot` now take `TestAccess.lock`, closing a data race in the test harness.** The evaluator writes `EvalSnapshot` (`failures`, `passedAccesses`, `capturedValueUpdates`) inside `TestAccess.lock`; `expect` then read two of those fields *without* the lock after `awaitPredicate` returned. The old doc-comment justified that with "the continuation-resume happens-before the await-return, so the final snapshot write is visible" — **which is wrong**. Resuming a continuation re-enqueues the awaiting task, and its executor is free to pick it up on a *different* thread than the one that ran the evaluator, leaving no edge between the write and the read. ThreadSanitizer reports it as `_evaluateExpect` (write, `TestExpect.swift:387`, under the lock) vs `TestAccess.expect` (read, no lock). The two offending reads — `capturedValueUpdates` on the `.passed` path and `failures` on the failure-reporting path — now copy out under the lock and work on the copy; `passedAccesses` was already read inside a `lock { }` block. The lock is *not* held across the reporting that follows, which would deadlock (`diffMessage` → `customMirror` → `willAccess` → the same non-cross-thread-reentrant lock).

  **The fix is unproven against a reproduction, and this entry does not claim otherwise.** The race was observed three times across two sessions, always with the same signature, but it is rare and its rate is not stable: an interleaved A/B (6 runs per arm, same probe load, same invocation, one session) saw **zero** occurrences on the *unfixed* baseline arm, so the six clean fixed runs establish nothing. The change is made because the synchronisation argument it replaces is demonstrably false, not because a red turned green. Test-harness only — no library behaviour change; the copied values are identical to what the unlocked reads produced, and the full suite is green serial, parallel, and under the TSan gate.

### Tests

- **`CancellableLockOrderDeadlockTests`** drives the H/T inversion above from both shapes that reach it: back-to-back subtree replacements inside a single transaction (the second teardown's `cancelAll` landing while the first rebuild's activation tasks are still registering), and sibling subtrees replaced together so the overlap happens between contexts rather than within one. A deadlock here does not fail the test, it hangs — so reaching the end is the assertion. Same smoke-test posture as `HierarchyLockOrderDeadlockTests` and `MemoizeLockOrderDeadlockTests`.

- **`DynamicMemberPathBenchmark`** measures the three read paths that route through `_ModelSourceBox.subscript(dynamicMember:)` — pre-anchor, snapshot, and `forceDirectAccess`. None of them were covered by `ReadPathBenchmarks`, whose every case is an anchored+tracked read through `context[statePath:observeCallback:]`; that gap is why the locked-projection fix above initially looked free when it was not.
...
- **The `swift-syntax` dependency is now a range, `"600.0.0"..<"605.0.0"`, instead of `from: "600.0.0"`.** swift-syntax bumps its *major* version once per Swift release (600 ≙ Swift 6.0, 601 ≙ 6.1, 602 ≙ 6.2, 603 ≙ 6.3), so `from: "600.0.0"` meant `600.0.0..<601.0.0` — a hard ceiling at Swift 6.0's swift-syntax that every consumer inherited. Because SwiftPM resolves package versions for the whole graph irrespective of platform conditions, that ceiling made SwiftModel unresolvable alongside any package requiring a newer swift-syntax (for example [swift-java](https://github.com/swiftlang/swift-java), which needs ≥ 603) on *every* platform, not just the one that pulled it in. The upper bound matches `swift-macro-testing` and `swift-snapshot-testing`, which are already in this graph declaring `"509.0.0"..<"605.0.0"`, so no single dependency is the binding constraint on swift-syntax. The macro plugin is verified against every major in the range: 600.0.1, 601.0.1, 602.0.0 and 603.0.2 all build warning-free and produce byte-identical macro expansions (every `SwiftModelMacroTests` snapshot passes unchanged on all four). Nothing about the minimum toolchain, the `swift-tools-version: 6.1` floor, or the `.macOS(.v11)` / `.iOS(.v14)` platform floors changes.
  ...
  Two source changes in the macro plugin were needed to compile across the range, neither of which affects generated code:

  - `ModelMacro`'s `ExtensionMacro` fallback path (used when `protocols` is empty, i.e. under MacroTesting) matched inheritance-clause entries with `String.contains(_:)` on the printed type. As of swift-syntax 603 that resolves to the macOS 13+ overload, which a `.macOS(.v11)` package can't use. It now compares the type's trailing identifier (`SwiftModel.Model` and `@unchecked Sendable` reduce to `Model` / `Sendable`), which is Foundation-free, availability-free, and no longer over-matches — `ModelContainer` used to read as a declared `Model` conformance.
  - Both `MemberMacro` conformances now implement `expansion(of:providingMembersOf:conformingTo:in:)`. swift-syntax 603 deprecates relying on the default that forwards to the `conformingTo:`-less form; the overload has existed since 600, so this compiles across the whole range. The added `protocols` parameter is unused — conformances are emitted by the `ExtensionMacro` conformance, which receives the same list.
...
- **Dropped the last `OneState` naming leftover, and acknowledged the predecessor in the README.** The macro plugin's `@main` type was still called `OneStateMacroPlugin`, carried over from [One State](https://github.com/bitofmind/swift-one-state); it's now `SwiftModelMacroPlugin`. The type is internal to the plugin executable and referenced nowhere else, so this is a pure rename with no build-plan or API effect. One State has been archived with a notice pointing here, and the README's Acknowledgements section now records the lineage.
...
- **The `withObservationTracking` gap-detector shadow now also covers synthetic-path dependencies (memoize sentinels, environment/local storage, preferences, the parents relationship).** The shadow `AccessCollector` — the persistent per-(context, path) subscription that closes Apple's one-shot registration gaps (Gaps A/B) — was dispatched only from `Context.willAccessDirect`, i.e. only for real `_State` reads. Synthetic reads inside an `observe()` body registered solely with Apple's one-shot tracking (`willAccessSyntheticPath`; `ModelContext.willAccess` short-circuits on `isInsideMemoizeObserve`), so a synthetic-dependency write landing between one-shot registration windows was dropped and the outer `Observed`/memoize held a stale value until some other tracked dependency changed — consistent with the documented `MemoizeTests` `.withObservationTracking` load flakes. The new `Context.willAccessGapShadow` dispatches the shadow at all four synthetic read sites, subscribing on the key path whose `modifyCallbacks` the write side actually fires (the typed `[_metadata:]`/`[_preference:]` paths for storage/preferences; the untyped sentinel/parents paths for memoize/parents). The synthetic write paths (`didModifyStorage`, `didModifyPreference`, `notifyPreferenceChange`, the memoize sentinel notification drains) also gained the same tier-2 `lockHeldBackgroundCalls` performUpdate-enqueue deferral that real writes get from `Context.transaction`, so the second listener cannot schedule duplicate performUpdates against a just-cleared `hasPendingUpdate` flag.
...
- **Cross-hierarchy dependency-dictionary and `rootPaths` races closed (the audit's deferred lock-design item).** These only bite when a separately-anchored model is grafted into another hierarchy (`child.lock !== parent.lock`) — the same family as the historical `addParent`/`removeParent` locking, iterative `didModify` walk, and `myModelRef` same-lock-guard fixes. Three holes: `nearestDependencyContext` read each *ancestor*'s `dependencyContexts` with no lock that ancestor's writers hold; `setupModelDependency` wrote `rootParent.dependencyContexts`/`dependencyCache` under only *self*'s hierarchy lock; and `rootPathTree()` read each parent's `children`/`collectionElementPathMakersStore` under only self's lock. Taking the other hierarchy's lock in place would reverse teardown's parent → child lock order (AB-BA deadlock), so instead: the two dependency dictionaries are now guarded by a dedicated per-context leaf lock (`dependenciesLock`, the `parentsLock` pattern — the stores are private and every access, own- or cross-hierarchy, goes through leaf-locked accessors; writers additionally keep the hierarchy lock they already held, and check-then-insert is atomic under the leaf lock so racing grafted writers can't double-link a dep context), and `rootPathTree()` now walks upward one lock at a time (each parent's snapshot taken under that parent's own lock, released before the next hop — `didModify`'s iterative-walk precedent), which also moves element-path-maker key-path construction outside every context lock. *Mutual* grafting (a parent-graph cycle) remains structurally unsupported — `rootParent`/ancestor walks would never terminate — and teardown deliberately does not defend against its theoretical teardown-vs-teardown AB-BA; this is now documented at `removeParent`.

- **`TestAccess._noteActivity` is now re-entrancy-safe (pre-existing crash, no library behavior change).** Its evaluation loop runs `expect`/`settle` predicates under the recursive `TestAccess` lock while doing positional `remove(at:)` on `_pendingExpects` — but a predicate *read* can synchronously re-enter `_noteActivity` on the same thread (reading a dirty memoized value on the AccessCollector path runs `entry.onUpdate` inline, whose sentinel notification fires a `registerReadOnlyPathWake` wake), and the inner call's removals left the outer loop's saved indices stale: `Fatal error: Index out of range`, or a silent wrong-entry removal. Reproduced on the unmodified library with the new `SyntheticPathGapShadowTests` stress. A same-thread re-entrancy guard now absorbs the inner call and reruns the full evaluation pass, so the inner activity's effects are still evaluated exactly once.

- **`SettleBudgetCapStarvationTests` control test no longer flakes under the `macOS (TSan)` CI job.** `normalDeferentialSettleResolvesOnQuietWindowAndCancelsBackstop` armed its budget backstop at a **fixed** 5 s from *registration* time but drove the manual tick with `DispatchTime.now()` at *tick* time — under TSan on a saturated runner the registration→tick gap measured > 5 s, so the backstop deadline was legitimately reached and both entries fired (`fired == 2`; red on `main`'s first TSan run as well). The budget and the suite's local pending-poll window now scale with `SWIFT_MODEL_TIMEOUT_SCALE` — the same treatment the TSan job's first runs applied to the three `ReactiveWaitInfrastructureTests` fixed constants (see the CI entry under *Added*).
...
- **CI: `macOS (TSan)` job — a ThreadSanitizer gate over the full parallel suite.** Runs `swift test --parallel --sanitize=thread` (`SWIFT_MODEL_TIMEOUT_SCALE=6` — TSan slows execution 5–15×) and fails on **any** `WARNING: ThreadSanitizer` in the log, printing the full reports (TSan does not fail the exit code by itself, so a log scan is the gate). Skips only the documented-unsupported Observable-interop test (`testObservedStreamWithModelAccessingObservable`), which races by design in its own test code. The suite has been TSan-clean since the concurrency-audit fixes above, so any report is a regression — this is the guard that keeps it that way. The job's first runs immediately caught three `ReactiveWaitInfrastructureTests` meta-tests using **fixed** wall-clock constants that TSan-grade slowdown exceeds — `withTestTimeout_cancelsBodyThroughWaitPrimitive`'s 5 s cancel-propagation margin, and `awaitPredicate_resolvesOnActivity` / `_resolvesOnCancellation`'s raw 5 s `awaitPredicate` deadlines, which race the test's own main task getting a post-`Task.sleep` cooperative-pool slot (measured > 5 s under TSan + parallel saturation). All three now scale with `SWIFT_MODEL_TIMEOUT_SCALE`, the same treatment as the 411ce44 cancel→resume bounds. The infrastructure itself was verified sound (`_awaitPending`'s initial evaluation, registration, and cancel-check are one lock scope — no missed wakeup).

### Changed

- **Removed the write-only `_PendingStackBox.latest` thread-local (internal cleanup, follow-up to 1.0.8's stale-`latest` fix).** Since that fix removed `_threadLocalStoreOrLatest`'s read path, `latest` was only ever written (`_threadLocalStoreAndPop` set it; `_threadLocalStoreFirst` and thread exit cleared it) — never read — and uselessly retained the last-constructed model's pre-anchor `Context.Reference` per thread until the next construction or thread exit. The property and its writes/clears are gone. No macro-emission change: the macro-called `_threadLocalStore*` entry points keep their names and signatures (the "OrLatest" name is baked into expanded client code; its doc comment records the history).
