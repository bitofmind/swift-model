...
- **`GlobalTickScheduler`** (internal) — a process-wide GCD-backed tick scheduler shared by every per-test `TestAccess`. Deadlines for `expect` / `require` / `settle` and the per-test wall-clock cap (`.modelTesting`'s 30 s trait timeout) are registered with the scheduler; it fires from GCD's thread pool, so the cooperative pool can be fully saturated with parallel-test work and the deadline still arrives on time. Hangs in the cooperative pool now surface as a deterministic `[TRAIT timeout]` failure rather than as wedged tests.
