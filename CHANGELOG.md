...
- **The executor-drive hang watchdog now scales with `SWIFT_MODEL_TIMEOUT_SCALE`.** `_executorHangDeadlineNs` was a fixed 120 s while every other infrastructure timeout multiplies by the scale knob. The drive's fixpoint check waits on, among other drains, the per-context observation queue that empties on the *one shared main thread* — and on a machine oversubscribed by several concurrent test processes (the same downstream report: six test bundles under parallel `xcodebuild`, each a saturated process) those drains can stretch a perfectly healthy settle past an unscaled 120 s, producing a false "never reached a fixpoint" that no `SWIFT_MODEL_TIMEOUT_SCALE` setting could relieve. The watchdog window (`_executorHangWindowNs`, also the absolute backstop `waitUntil` uses under the drive) is now `120 s × scale`, keeping its ordering invariants at every scale — above the trait cap's inactivity window (`30 s × scale`), below the trait's absolute deadlock ceiling (`300 s × scale`) — which `hangWatchdogStaysBetweenTraitWindowAndCeiling` pins.
