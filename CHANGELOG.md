...
- **Off-main writes to a main-tracked model now queue one main-registrar notification per (context, property) between main-actor drains, instead of one closure per write.** Every model a SwiftUI view has rendered has a *main* `ObservationRegistrar`, and each write to it from another thread must hand that registrar's `willSet`/`didSet` to `MainCallQueue` for delivery on the main actor. That used to be a closure allocation, a lock and an array append per write — and while the main thread was busy (layout, a long body, a synchronous load) the array simply grew, with `_ArrayBuffer._consumeAndCreateNew` copying it on every append. `MainCallQueue.notifyRegistrar` now keeps a pending set keyed by context identity and observer-key-path identity (the key paths are cached per property, so identity is stable), drops repeats at enqueue time with no allocation, and delivers the distinct pairs in first-insertion order at the point the first of them was queued relative to any other closures. The queue is bounded by the number of distinct properties written between drains, not by the number of writes. Collapsing repeats is safe because Apple's observation is one-shot per registration, and every coalesced write is already applied before its pair is queued, so a registration made after the first delivery has already read the state of all of them — the reasoning is written up at `notifyRegistrar`. Measured in Release on an M1 Max with the new `SwiftModelBenchmarks --burst` probe (100 k off-main writes to one main-tracked property): with the main thread blocked for the burst, **337 µs per write → 1.44 µs** (33.7 s → 144 ms), queue depth 100 k → 1; with main draining concurrently, ~3.5 µs → ~3.2 µs per write. Batched writes (`_withBatchedUpdates`) go through the same path.

  `drainIfOnMain()` — which every main-thread write calls to flush notifications bridged over from other threads — now checks a lock-free flag (`Synchronization.Atomic` on macOS 15 / iOS 18+, a locked `Bool` on older runtimes) and returns without touching the queue's lock or asking for the current thread when nothing is queued, which is nearly always. That was ~10 % of a single-threaded write in the profile; the section-2 `property write` row went from 1600 to 1494 ns best-of-3.

  Two probe corrections that turned up on the way: the `--contention` / `--profile` modes used `dispatchMain()`, which parks the real main thread and hands the main queue to a dispatch worker on which `Thread.isMainThread` is false — so no model in those probes ever had a main registrar and the off-main enqueue path was never exercised by them; they now drive the main queue from the main thread's run loop. And new `c4m … main-tracked` rows measure writes to models that do have one: those cost ~3.1 µs per write on one thread against ~1.25 µs untracked, and scale to ~50 µs at 8 threads on one tree, because the writer and the main-thread drain contend for the queue lock and the tree lock — the enqueue itself is now a small part of that, and the remainder is a separate piece of work.
