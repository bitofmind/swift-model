...
- **`settle()` (and `expect()`) no longer report a false `settle() timed out: model still has active tasks` failure when the model is actually quiescent but the host is under cross-process CPU saturation.** In-test settle arms its quiet-window confirmation with `.deferential` priority, which `GlobalTickScheduler.fire()` dispatches to `DispatchQueue.global(qos: .background)`. When another process saturates the cores (the classic case: a second `xcodebuild`/`swift-frontend` running on the same self-hosted CI host), macOS starves that `.background` slot — so even though the model has been idle the whole time (bg drained, no active tasks), the quiet window is never *confirmed*. The `.responsive` budget backstop then trips the total-budget cap and `waitUntilSettled` judged the resolution purely by `now >= budgetEnd`, reporting a timeout whose diagnostic active-task list is empty (the misleading message), then `-retry-tests-on-failure` re-runs the test into the same starvation and the job blows its wall-clock budget. The fix re-checks *actual* quiescence inline at the budget cap — the same conditions the quiet-window callback would have checked had it been scheduled: `backgroundCall.isIdle`, no task registered-but-not-yet-started (`!context.hasPendingStartTask`), and no model activity for at least one debounce window (tracked via a new `lastActivityNs` stamp in `_noteActivity`, seeded at wait-arm time). If all hold, the settle succeeded and only its confirmation was late, so it returns success; a genuinely churning model (recent activity), a busy bg pipeline, or a task still pending its first run still fails exactly as before. No timeout budgets changed. Regression coverage in `SettleBudgetCapStarvationTests` (`inTestSettleSucceedsWhenQuiescentButDeferentialPrimaryStarved` + the over-correction guard `inTestSettleStillFailsWhenActivityRecentAtBudget`).
